...
public class ProfitTableSheetCal {
    ...
    public void UpdateProfitTable(String time, long company_id) {
        ...
        for (VoucherItem item : list){
            System.out.println(item.getVoucherId()+ " " +item.getSubjects());
        }
        ...
        save(company_id, time, "一、营业收入", business_income);

        double business_costs = 0;//营业成本
        temp1 = th.Cal2("5401", map);
        temp2 = th.Cal2("5402", map);
        business_costs = temp1 + temp2;//主营业务成本+其他业务成本
        save(company_id, time, "减：营业成本", business_costs);
        ...
        save(company_id, time, "其中：消费税", Business_Taxes_and_Surcharges[1]);
        save(company_id, time, "营业税", Business_Taxes_and_Surcharges[2]);
        save(company_id, time, "城市维护建设税", Business_Taxes_and_Surcharges[3]);
        save(company_id, time, "资源税", Business_Taxes_and_Surcharges[4]);
        save(company_id, time, "土地增值税", Business_Taxes_and_Surcharges[5]);
        save(company_id, time, "城镇土地使用税、房产税、车船税、印花税", Business_Taxes_and_Surcharges[6]);
        save(company_id, time, "教育费附加、矿产资源补偿费、排污费", Business_Taxes_and_Surcharges[7]);
        ...
        save(company_id, time, "销售费用", Selling_expenses[0]);
        save(company_id, time, "其中：商品维修费", Selling_expenses[1]);
        save(company_id, time, "广告费和业务宣传费", Selling_expenses[2]);
        ...
        save(company_id, time, "管理费用", Management_expenses[0]);
        save(company_id, time, "其中：开办费", Management_expenses[1]);
        save(company_id, time, "业务招待费", Management_expenses[2]);
        save(company_id, time, "研究费用", Management_expenses[3]);
        ...
        save(company_id, time, "财务费用", Financial_expenses[0]);
        save(company_id, time, "其中：利息费用（收入以“-”号填列）", Financial_expenses[1]);


        double investment_proceeds = th.Cal("5111", map);//投资收益
        save(company_id, time, "加：投资收益（亏损以“-”号填列）", investment_proceeds);
        ...
        save(company_id, time, "二、营业利润（亏损以“-”号填列）", operating_profit);
        ...
        save(company_id, time, "加：营业外收入", Non_operating_income[0]);
        save(company_id, time, "其中：政府补助", Non_operating_income[1]);
        ...
        save(company_id, time, "减：营业外支出", Non_operating_expenses[0]);
        save(company_id, time, "其中：坏账损失", Non_operating_expenses[1]);
        save(company_id, time, "无法收回的长期债券投资损失", Non_operating_expenses[2]);
        save(company_id, time, "无法收回的长期股权投资损失", Non_operating_expenses[3]);
        save(company_id, time, "自然灾害等不可抗力因素造成的损失", Non_operating_expenses[4]);
        save(company_id, time, "税收滞纳金", Non_operating_expenses[5]);

        double Total_profit = operating_profit + Non_operating_income[0] - Non_operating_expenses[0];
        save(company_id, time, "三、利润总额（亏损总额以“-”号填列）", Total_profit);

        double Income_tax_expense = th.Cal2("5801", map);
        save(company_id, time, "减：所得税费用", Income_tax_expense);
    ...
    private void save(Long companyId, String time, String name, Double value) {
        ...
        if (item != null) {
            ...
            item = new ProfitSheet(companyId, time, name, value);
            psr.save(item);
        }
    }
}
